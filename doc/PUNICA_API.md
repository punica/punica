...
**Register device**
----
  Register a new device. A new device entry is appended to the existing database. If database file name is provided, database is saved to that file. Response includes base64 encoded security credentials for the given security mode and a unique identifier or 'uuid' that is used to refer to said device entry, both generated by the server.
