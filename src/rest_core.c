...
static char *logging_section = "[REST_API]";

int rest_step(punica_context_t *punica, struct timeval *tv_interval)
{
    struct _u_request u_request;
    struct _u_response u_response;
    struct _u_map u_headers;
    json_t *j_body, *j_headers, *j_value;
    ...
    {
        url = json_string_value(
                  json_object_get(punica->j_rest_callback, "url"));
        j_headers = json_object_get(punica->j_rest_callback, "headers");
        ...
        log_message(LOG_LEVEL_INFO, "%s Sending callback to \"%s\"\n",
                    logging_section, url);
        ...
        u_request.client_cert_file =
            o_strdup(punica->settings->http.security.certificate);
        u_request.client_key_file =
            o_strdup(punica->settings->http.security.private_key);

        if ((punica->settings->http.security.certificate != NULL
             && u_request.client_cert_file == NULL)
            || (punica->settings->http.security.private_key != NULL
                && u_request.client_key_file == NULL))
        {
            log_message(LOG_LEVEL_ERROR,
                        "%s Failed to set client security credentials on callback!\n",
                        logging_section);
